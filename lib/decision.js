...
class Decision {
    ...
    __DECISIONTABLE(dmn) {
        let node = new Node({ node: Node.DECISIONTABLE, name: dmn.name, hitPolicy: dmn.hitPolicy, aggregation: dmn.aggregation ,inputs: [], outputs: dmn.outputs, rules: [] });
            ...
            input.forEach((input) => {
                if (input.inputExpression) {
                    node.inputs.push({
                        ...
                        name: input._inputVariable || input._label || input.inputExpression.text || undefined,
                        type: input.inputExpression._typeRef || undefined,
                        expression: input.inputExpression.text || undefined 
                    }) 
                }
            })
            // can be array or single -> convert to array
