...
class Decision {
    ...
    parseFeel(expression) {
        ...
        try {
            ...
            interpreter.logger.error({
                type: "ParseError",
                message: "Can't parse input",
                expression: expression, 
                error: e
            });
            return [new Node({ node: Node.NULL })];
        }
    }
    ...
    evaluate({ decision, data } = {}) {
        ...
        interpreter.setDecision(this.decision || null );
        this.result = interpreter.evaluate({ context: this.data });
        return this.result;
    }

    analyse({ decision, data, level = 'D' } = {}) {
        this.data = {};
        if (decision || data) {
            this.decision = decision || this.decision;
            this.data = data || {};
        ...
        this.result = null;
        interpreter.ast = this.ast;
        interpreter.logger.activate(level);
        interpreter.logger.clear();
        interpreter.setDecision(this.decision || null );
        this.result = interpreter.evaluate({ context: this.data });
        return {
    ...
    __INPUT(dmn) {
        ...
        let node = new Node({ node: Node.INPUT, name: dmn.name, varType: dmn.variable?.varType, value: String(dmn.name) });
        return node;
    }
    ...
    __DECISIONTABLE(dmn) {
        ...
        dmn.rules.forEach((element,index) => {
            ...
            element.inputs.forEach((input,index) => {
                ...
                let test = new Node({ node: Node.IN, input: inputs[index].value, inputType: inputs[index].type, test: ast[0] });
                rule.inputs.push({
