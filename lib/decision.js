...
class Decision {
    ...
    parse({ xml }) {
        if (!xml) return false;
        let node = new DMNParser().parse(xml);
        console.log(util.inspect(node, { showHidden: false, depth: null, colors: true }));
        if (!node) return false;
        this.ast = this._build(node);
        
        return true;
    }

    evaluate({ ast = null, context = {} }) {
        this.result = null;
        if (ast) this.ast = ast;
        if (!this.ast) return this.result;
        this.result = interpreter.evaluate();
    ...
    _build (node) {
        ...
        if (this["__"+node.type] && {}.toString.call(this["__"+node.type]) === "[object Function]") {
            return this["__"+node.type](node);
        } else {
            //console.log("Interpreter - missing function ", node, this.expression);
            throw new Error("Interpreter - missing function " + node.type, { node, partlyBuild: this.ast });
        }
    }
    ...
    __DECISIONTABLE(node,context) {
        ...
        this.ast =  interpreter.parse("1 + 1");
        return;
