...
class Interpreter {
    ...
    __DATE_AND_TIME (node,context) {
        ...
        try {
            ...
            // build parameters
            if (node.parameters.node === Node.LIST) {
                if (node.parameters.entries?.length === 1) {
                    parameters = this.buildParameters(node.parameters,["from"],context);
                } else if (node.parameters.entries?.length === 3 && node.name === 'date') {
                    parameters = this.buildParameters(node.parameters,["year","month","day"],context);
                } else if (node.parameters.entries?.length === 3 && node.name === 'time') {
                    parameters = this.buildParameters(node.parameters,["hours","minutes","seconds"],context);
                } else if (node.parameters.entries?.length === 4 && node.name === 'time') {
                    parameters = this.buildParameters(node.parameters,["hours","minutes","seconds","offset"],context);
                } else if (node.parameters.entries?.length === 2 && node.name === 'date and time') {
                    parameters = this.buildParameters(node.parameters,["date","time"],context);
                } else if (node.parameters.entries?.length === 3 && node.name === 'date and time') {
                    parameters = this.buildParameters(node.parameters,["year","month","day"],context);
                }
            } else {
                parameters = this._build(node.parameters,context);
            }
            if (Array.isArray(parameters) && parameters.length === 3 && node.name === 'date') {
                result =  DateOnly.build(parameters);
            } else if (Array.isArray(parameters) && parameters.length >= 3 && node.name === 'time') {
                result = Time.build(parameters);
            } else if (parameters !== null && typeof parameters === 'object' && parameters.hours && parameters.minutes && parameters.seconds && parameters.offset) {
                result = Time.build([parameters.hours, parameters.minutes, parameters.seconds]).add(parameters.offset);
            } else if (parameters !== null && typeof parameters === 'object' && parameters.hours && parameters.minutes && parameters.seconds) {
                result = Time.build([parameters.hours, parameters.minutes, parameters.seconds]);
            } else if (parameters !== null && typeof parameters === 'object' && parameters.year && parameters.month && parameters.day) {
                if (Temporal.isValidDate(parameters.year, parameters.month, parameters.day)) {
                    result = DateOnly.build([parameters.year, parameters.month, parameters.day]);
                }
            } else if (parameters !== null && typeof parameters === 'object' && parameters.year && parameters.month && parameters.day && parameters.hour && parameters.minute) {
                if (Temporal.isValidDate(parameters.year, parameters.month, parameters.day)) {
                    result = DateAndTime.build([parameters.year, parameters.month, parameters.day, parameters.hour, parameters.minute]);
                }
            } else if (parameters !== null && typeof parameters === 'object' && parameters.date && parameters.time) {
                if (parameters.date instanceof DateOnly && parameters.time instanceof Time) {
                    result = DateAndTime.build([parameters.date.year, parameters.date.month, parameters.date.day, parameters.time.hour, parameters.time.minute, parameters.time.second]);
                } else if (parameters.date instanceof DateAndTime && parameters.time instanceof Time) {
                    result = DateAndTime.build([parameters.date.year, parameters.date.month, parameters.date.day, parameters.time.hour, parameters.time.minute, parameters.time.second]);
                }
            } else if (parameters !== null && typeof parameters === 'object' && parameters.from) {
                if (parameters.from instanceof DateOnly) {
                    result = parameters.from;                    
                } else if (parameters.from instanceof DateAndTime && node.name === 'date') {
                    result = parameters.from.toDateOnly();
                } else if (parameters.from instanceof DateAndTime) {
                    result = parameters.from;
                } else  {
                    result = Temporal.parse(parameters.from);
                }
            } else  {
                result = Temporal.parse(parameters ? parameters[0] : null);
            }
        } catch (e) {
            this.logger.error({
                type: "Date and time",
            ...
            switch (node.filter.node) {
                ...
                case Node.COMPARISON:
                    ...
                    if (node.filter.operator === "=" && node.filter.left.node === Node.NAME && node.filter.left.value.toUpperCase() === "KEY") {
                        ...
                        let check = false;
                        if (list[0].x === 1 && list[0].y === 2)    check = true;  
                        if (check) console.log("_FILTER",util.inspect(list, { showHidden: false, depth: null, colors: true }));
                        list.forEach((item) => {
                            //console.log("_FILTER",util.inspect(list, { showHidden: false, depth: null, colors: true }));
                            if (check) console.log("_FILTER",util.inspect(item, { showHidden: false, depth: null, colors: true }));
                            if (check) console.log("_FILTER context",util.inspect({ ...item, item }, { showHidden: false, depth: null, colors: true }));
