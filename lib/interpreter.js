...
class Interpreter {
    ...
    __EXPONENTATION (node,context) {
        let result;
        const base = new Decimal(this._build(node.left,context));
        const exponent = new Decimal(this._build(node.right,context));
    ...
    __FUNCTION_CALL (node,context) {
        ...
        switch (node.name.value) {
            ...
            case "number": {
                ...
                return typeof parameters.from == 'string' ? new Decimal(parseFloat(parameters.from)) : undefined;
            }
            case "string": {
                let value;
                const parameters = this.buildParameters(node.parameters,["from"],context);
            ...
            case "substring": {
                ...
                const start = this.toNumber(parameters.start);
                const length = this.toNumber(parameters.length);
                return (typeof parameters.string == 'string') ? parameters.string.substring(start-1, length ? start+length-1 : parameters.string.length) : undefined;
            }
            case "string length": {
            ...
            case "stddev": {
                ...
                const mean = parameters.reduce((a, b) => a.add(b)).div(n);
                const variance = parameters.map(x => new Decimal(x.sub(mean).pow(2))).reduce((a, b) => new Decimal(a.plus(b))).div(n);
            ...
            case "floor": {
                // too many parameters -> null
                if (node.parameters?.entries?.length > 1) throw new Error("Too many parameters for floor function");
                ...
                const result = parameters.n.floor();
                this.logger.debug({
                    type: "function floor",
                    parameters,
                    result
            ...
            case "ceiling": {
                // too many parameters -> null
                if (node.parameters?.entries?.length > 1) throw new Error("Too many parameters for ceiling function");
                const parameters = this.buildParameters(node.parameters,["n"],context);
                // undefined or null -> null
                ...
                const result = parameters.n.ceil();
                this.logger.debug({
            ...
            case "round up": {
                ...
                const parameters = this.buildParameters(node.parameters,["n","scale"],context);
                const scale = parameters.scale ? this.toNumber(parameters.scale) : 0;
            ...
            case "round down": {
                ...
                if (typeof scale !== 'number') throw new Error(`function round down: parameter scale "${parameters.scale}" is not a number`);
            ...
            case "round half up": {
                ...
                if (typeof scale !== 'number') throw new Error(`function round half up: parameter scale "${parameters.scale}" is not a number`);
            ...
            case "round half down": {
                ...
                if (typeof scale !== 'number') throw new Error(`function round half down: parameter scale "${parameters.scale}" is not a number`);
            ...
            case "modulo": {
                ...
                const result = parameters.dividend.mod(parameters.divisor);
                this.logger.debug({
            ...
            case "log": {
                ...
                const result = parameters.number instanceof Decimal ? parameters.number.ln() : new Decimal(parameters.number).ln();
                this.logger.debug({
                    type: "function log",
                    parameters,
                    result
