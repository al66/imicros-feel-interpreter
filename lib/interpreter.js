...
class Interpreter {
    ...
    convertTo(value,type) {
        ...
        throw new Error("Cannot convert value to type " + type + ": " + value);
    }
    ...
    __COMPARISON (node,context) {
        ...
        if ((typeof left === 'string' && typeof right !== 'string') || (typeof left !== 'string' && typeof right === 'string')) {
            throw new Error(`Cannot compare string and other types - operator: ${ node.operator }, left: ${left}, right: ${right}`);
        }
        ...
        switch (node.operator) {
            case "=": {
                if (left instanceof Decimal && right instanceof Decimal) {
                    ...
                    result = _.objectAreEqual(left,right);
                } else {
                    result = left == right;
                }
                break;
    ...
    __LOGICAL (node,context) {
        ...
        const right = this._build(node.right,context);
        let result = null;
        // check if left is valid type
        if (left !== null && typeof left !== 'boolean') throw new Error(`Cannot evaluate logical with type ${this.getType(left)} of left value`);
        // check if right is valid type
        if (right !== null && typeof right !== 'boolean') throw new Error(`Cannot evaluate logical with type ${this.getType(right)} of right value`);
        // Ternary logic
        switch (node.operator) {
            case "and": {
                if (left === null && right === null) {
                    result = null;
                } else if (right === null && left === false) {
                    result = false;
    ...
    __CONTEXT (node,context) {
        ...
        if (node.data && Array.isArray(node.data.entries)) {
            node.data.entries.forEach((entry) => {
                ...
                if (element) {
                    ...
                    if (value[element.key] !== undefined) {
                        throw new Error(`Key "${element.key}" already defined in context`);
                    }
        ...
        let result = {};
        // if context in a decision, a variable may be defined
        if (node.variable && node.variable.name) {
            result[node.variable?.name] = value;
        } else {
            result = value;
        }
        this.logger.add({
    ...
    __IN (node,context) {
        ...
        switch (node.test.node) {
            ...
            case Node.FUNCTION_CALL: {
                ...
                } else if (input instanceof Temporal && typeof value === 'string') {
                    const tmp = Temporal.parse(value);
                    result = input.value == tmp.value;                    
            ...
            switch (node.operator) {
                case "!=": {
                    if (input instanceof Decimal && value instanceof Decimal) {
                        ...
                        const date = Temporal.parse(value);
                        result = input.value !== value.value;
                    } else {
                        result = input !== value;
                    }
    ...
    __MAIN (node,context) {
        ...
        for (let i = 0; i < node.definitions.length; i++) {
            ...
            } else if (output && this.decision && this.decision !== node.definitions[i].name) {
                this.logger.debug({
                    type: "Decision",
                    filtered: true,
    ...
    __INPUT (node,context) {
        ...
        context[node.name] = structuredClone(this.data[node.value]);
        if (node.varType === "number" && typeof context[node.name] === "string") {
            context[node.name] = parseFloat(context[node.name]);
        }
        if (node.varType === "date" && typeof context[node.name] === "string") {
            context[node.name] = Temporal.parse(context[node.name]);
        }
        if (node.varType === "time" && typeof context[node.name] === "string") {
            context[node.name] = Temporal.parse(context[node.name]);
        }
        if ((node.varType === "date-time" || node.varType === "dateTime" ) && typeof context[node.name] === "string") {
            context[node.name] = Temporal.parse(context[node.name]);
        }
        if ((node.varType === "days and time duration" || node.varType === "dayTimeDuration" ) && typeof context[node.name] === "string") {
            context[node.name] = Temporal.parse(context[node.name]);
        }
        if ((node.varType === "years and months duration" || node.varType === "yearMonthDuration" ) && typeof context[node.name] === "string") {
            context[node.name] = Temporal.parse(context[node.name]);
        }
    ...
    __DECISIONTABLE (node,context) {
        ...
        switch (node.hitPolicy) {
            ...
            case "Unique": {
                ...
                // return as variable, if variable is defined and is not equal output column
                if (node.variable?.name) {
                    if (outputs.length > 1) {
                        output[node.variable.name] = rulesOutput;
                    } else if (outputs.length === 1 && outputs[0].name !== node.variable.name) {
                        output[node.variable.name] = rulesOutput;
                    } else {
                        output = rulesOutput;
                    }  
    ...
    __FUNCTION_CALL (node,context) {
        ...
        switch (node.name.value) {
            ...
            case "split": {
                // too many parameters -> null
                if (node.parameters?.entries?.length > 2) throw new Error("Too many parameters for split function");
                const parameters = this.buildParameters(node.parameters,["string","delimiter"],context);
                // check if patameter string is defined -> if not, return null
                if (!parameters.string) throw new Error("function split: parameter string is missing");
                // parameter string must be type string
                if (typeof parameters.string !== 'string') throw new Error("function split: parameter string is not a string");
                // check if delimiter is defined
                if (parameters.delimiter === undefined || parameters.delimiter === null) throw new Error("function split: parameter delimiter is missing");
                // check if delimiter is a string
                if (typeof parameters.delimiter !== 'string') throw new Error("function split: parameter delimiter is not a string");
                const result = Strings.split(parameters);
                this.logger.debug({
                    type: "function split",
                    parameters,
                    result
            ...
            case "product": {
                const parameters = node.parameters?.entries?.length > 1 ? [...this._build(node.parameters)] : this.buildParameters(node.parameters,["list"],context).list;
                if (!Array.isArray(parameters) && !(parameters instanceof Decimal)) throw new Error("function mean: parameter is not a list or a list of numbers");
                let result
                // handle single number -> return number
                if (parameters instanceof Decimal) {
                    result = parameters;
                } else {
                    result = new Decimal(parameters.reduce((a,b)=>a.mul(b)));
                }
                this.logger.debug({
