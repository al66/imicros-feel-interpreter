    ...
    decisionTable(node,context) {
        ...
        parameters["rule list"].forEach((rule, indexRule) => {
            ...
            parameters["inputs"].forEach((input, indexInput) => {
                ...
                // console.log(rule.entries[indexInput]);
                if (rule.entries[indexInput]?.node === Node.LIST) {
                    result.result = result.result && this._build(new Node({ node: Node.IN_LIST, input: new Node({ node: Node.NAME, value: this._build(input)}), list: rule.entries[indexInput]}),context)
                } else {
