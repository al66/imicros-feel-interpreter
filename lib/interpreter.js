...
class Interpreter {
    ...
    parse (exp) {
        ...
        try {
            ...
            this.error = {
                exp,
                number: e.token?.offset ? 100 : 900, // 100 unexpected character, 900 undefined parsing error
                text: e.token?.text || null,
                position: exp.substring(e.token?.offset > 10 ? e.token?.offset - 10 : 0, e.token?.offset || 0),
    ...
    __PRODUCT (node,context) {
        switch (node.operator) {
            ...
            // case "*": return this._build(node.left,context) * this._build(node.right,context); break;
            case "/": return new Decimal(this._build(node.left,context)).div(new Decimal(this._build(node.right,context))).toNumber(); break;
            case "*": return new Decimal(this._build(node.left,context)).mul(new Decimal(this._build(node.right,context))).toNumber(); break;
        }
    }
    ...
    __CONTEXT_ENTRY (node,context) {
        ...
        return { key: node.key.node === Node.NAME ? node.key.value : this._build(node.key,context), value: this._build(node.expression,context) };
    }
        ...
        parameters["rule list"].forEach((rule, indexRule) => {
            ...
            parameters["inputs"].forEach((input, indexInput) => {
                ...
                if (rule.entries[indexInput]?.node === Node.LIST) {
                    result.result = result.result && this._build(new Node({ node: Node.IN_LIST, input: inputNode, list: rule.entries[indexInput]}),context)
                    // console.log(new Node({ node: Node.IN_LIST, input: inputNode, list: rule.entries[indexInput]}),this.decisionContext,result.result);
                } else {
                    result.result = result.result && this._build(new Node({ node: Node.IN, input: inputNode, test: rule.entries[indexInput]}),context)
