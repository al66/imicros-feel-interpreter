...
class Interpreter {
    ...
    __SUM (node,context) {
        ...
        if (left === null || right === null) throw new Error(`Cannot evaluate sum with null values - operators: ${ node.operator }, left: ${left}, right: ${right}`);
        switch (node.operator) {
            case "-": {
                if (left instanceof Temporal) {
                    ...
                    result = bigmath.number(bigmath.subtract(bigmath.bignumber(left),bigmath.bignumber(right)));
                } else {
                    result = left - right;
                }
                break;
            ...
            case "+": {
                if (left instanceof Temporal) {
                    ...
                    result = bigmath.number(bigmath.add(bigmath.bignumber(left),bigmath.bignumber(right)));
                } else {
                    //if (math.typeOf(left) !== 'BigNumber' && typeof left === 'number') left = bigmath.bignumber(left);
                    //if (math.typeOf(right) !== 'BigNumber' && typeof right === 'number') right = bigmath.bignumber(right);
                    result = left + right;
                } 
                break;
            }
        }
        if (result == null) throw new Error(`Cannot evaluate sum with these values- operators: ${ node.operator }, left: ${left}, right: ${right}`);
        this.logger.debug({
            type: "Sum",
