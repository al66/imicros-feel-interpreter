...
class DMNCompiler {
    ...
    _build (node,context) {
        ...
        if (this["__"+node.type] && {}.toString.call(this["__"+node.type]) === "[object Function]") {
            ...
            throw new Error("Interpreter - missing function " + node.type);
        }
    }
    ...
    __INPUT(node,context) {
        this.lines.push({ 
            indent: this.indent, 
            text: `"${node.variable.name}": ${node.variable.name}${context.more ? ",\n" : "\n"}`
        });
    }
    ...
    __RETURN(node,context) {
        this.lastName = node.variable?.name || "__return";
        this.lines.push({
            indent: this.indent, 
            text: `"${this.lastName}": ${node.expression}${context.more ? ",\n" : "\n"}`
        })
    }
    ...
    __CONTEXT(node,context) {
        ...
        this.lines.push({ 
            indent: --this.indent, 
            text: `}${this.lastName ? "." + this.lastName : ""}${context.more ? "," : ""}\n` 
        });
        this.lastName = null;
    }
    ...
    __DECISIONTABLE(node,context) {
        function checkList(p) {
            if (typeof p === 'string') {
                // TODO: verify all possible expressions with comma inside ...
                if (p.match(/.*,.*/g)) return `[${p}]`;
            }
            return p;
        }
        // function start
        this.lines.push({ 
            indent: this.indent++, 
            text: `"${node.variable.name}": decision table(\n`
        });
        // inputs
...
const expression = new DMNCompiler().compile(ast);
